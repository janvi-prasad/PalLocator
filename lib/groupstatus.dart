  ...
  grps.forEach((k,v) async{
    ...
    if(v["groupname"]==groupStatusGroupname) {
      for(var i=0;i<v["members"].length;i++) {
        var response1 = await http.get(Uri.parse("https://pallocator-8fe3e.firebaseio.com/groups/${k}/members/${i}.json"));
      ...
      if(result1["emailid"]==loggedinUser){
        ...
        await httpClient.put(
            url: "https://pallocator-8fe3e.firebaseio.com/groups/${k}/members/${i}/locationShare.json",body: result2);
...
// Platform messages may fail, so we use a try/catch PlatformException.
    ...
    if (loggedInuserKey!="") {
      ...
      await httpClient.put(
          url: "https://pallocator-8fe3e.firebaseio.com/users/${loggedInuserKey}/location.json",
          body: locUpdated);
    }
    final DataSnapshot groupresmap=await getGroups();
    Map grps = groupresmap.value as Map;
        ...
        for(var i=0;i<v["members"].length;i++) {
          ...
          if(result1["emailid"]==loggedinUser){
            if(result1["locationShare"]==true||result1["location"]==null) {
              await httpClient.put(
                  url: "https://pallocator-8fe3e.firebaseio.com/groups/${k}/members/${i}/location.json",body: locUpdated);
            }
